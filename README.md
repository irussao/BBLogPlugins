...
<br>Info: Display a detailed scoreboard of the battlereport with BF4DB and BF4 Cheater report links for each player on the battlereport(Not compatible with "Friends Highlighter", it's already included), if you have the old "Anti-Cheat Links" plugin, then replace it with this one. There are settings to remove anti-cheat links and display just the basic stats.
